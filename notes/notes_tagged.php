...
if($uidv=="tagged"){$from_news_feed=''; $uidv=$uid;}
...
<div>
<h2 class="uiHeaderTitle"><i class="uiHeaderImage notessp_notes"></i>Notes About Me</h2>
</div>
</div>
</div>';
}
...
$echo.='
<span class="h1h">'.$uti['fullname'].'</span><i class="profileheader_i"></i><span class="h1h">Notes About</span>
</h1>
</div>
...
$trn=0;
$r=mysql_query("SELECT * FROM nta WHERE tagged='$uidv' AND (visibility='v' OR visibility='n')");
while($m=mysql_fetch_array($r)){
$sbid=$m['noteid'];
$r2=mysql_query("SELECT * FROM nt as dt WHERE sbid='$sbid' $a_qf");
$trn=$trn+mysql_num_rows($r2);
}
...
$pr=mysql_query("SELECT * FROM nta WHERE tagged='$uidv' AND (visibility='v' OR visibility='n') ORDER BY datetimep DESC LIMIT $gsn,10");

while($pm=mysql_fetch_array($pr)){
...
$r=mysql_query("SELECT * FROM nt as dt WHERE sbid='$sbid' AND id='$uidv' AND (visibility='v' OR visibility='n') $a_qf");
...
if($uti['id']!=$uid){$echo.='&#183; <a href="/'.$uti['username'].'?sk=notes">More from '.$uti['f_name'].'</a>';}
...
$vrt=mysql_fetch_array(mysql_query("SELECT COUNT(*) AS vrt FROM commentsv WHERE elemid='$thefabtext' AND visibility='v' AND type='$ltype'"));
$vrt=$vrt['vrt'];
...
$echo.='<div class="clearfix sbxNullState"><img style="display: block;margin-right: 8px;float:left;" height="32" width="32" src="/images/e4B6gExrCqw.png"><div style="display: table-cell;vertical-align: top;"><p style="font-size: 13px;padding:0;margin:0;padding-top: 7px;">No one has written any notes.</p></div></div>';	
}
...
$("#llmnt").removeClass("wrapper_fotos");
$("#llmnt").addClass("wrapper_fotos2");
$("#llmntv").removeClass("linkwrap");
$("#llmntv").addClass("linkwrap2");
</script>
';
}
else{
$params['left_link_w']='nt';
$params['left_menu_added']='
<script type="text/javascript" data-lma="">
