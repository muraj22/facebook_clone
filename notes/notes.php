...
if($uidv=="me"){$from_news_feed=''; $uidv=$uid;}
...
.idivi{
    ...
    background-image: url("/images/CVxs0AbYzXk.png");
    background-size: auto auto;
    background-repeat: no-repeat;
    display: inline-block;
}
...
if((isset($from_news_feed) OR isset($from_notes_php)) AND $uidv==$uid){
...
<div>
<h2 class="uiHeaderTitle"><i class="uiHeaderImage notessp_notes"></i>My Notes</h2>
</div>
</div>
</div>';
}
else if(isset($from_notes_php)){
$echo.='<div class="contentarea">
<div class="uiHeaderPage uiHeaderWithImage uiHeaderBottomBorder" style="padding-bottom:24px;">
<div class="clearfix uiHeaderTop">
<div style="float:right;">
<a href="/editnote.php" class="write_a_note uiButton"><i class="write_a_note_i"></i><span>Write a Note</span></a>
</div>
<div>
<h2 class="uiHeaderTitle"><i class="uiHeaderImage notessp_notes"></i>Notes<i class="idivi mhs"></i>'.$uti['f_name'].'\'s Notes</h2>
</div>
</div>
</div>';
}
...
$echo.='
<span class="h1h">'.$uti['fullname'].'</span><i class="profileheader_i"></i><span class="h1h">Notes</span>
</h1>
</div>
...
function custom_substr($string,$start,$end){
...
$r=mysql_query("SELECT * FROM nt as dt WHERE id='$uidv' AND (visibility='v' OR visibility='n') $a_qf");
...
$r=mysql_query("SELECT * FROM nt as dt WHERE id='$uidv' AND (visibility='v' OR visibility='n') $a_qf ORDER BY datetimep DESC LIMIT $gsn,10");
if($trn==0){
$echo.='
...
/* viejo : $bodyl=check_body_length($body,$allowed_sbeditor);
if(strlen($body)>354){
$dn=354+$bodyl;
$body=substr($body,0,$dn);
$body=$body.'...';	
}
*/
...
$r2=mysql_query("SELECT * FROM notes_pics WHERE noteid='$sbid' AND (visibility='v' OR visibility='n') AND id='$uidv' ORDER BY datetimep ASC");
...
<div class="note_title" style="display:inline-block;"><a href="/notes/'.$flm.'/'.$note_titlem.'/'.$sbid.'">'.$note_title.'</a></div>';
...
$echo.='<ul class="uiList inlineBlock rfloat ocontrol" style="opacity:0;position:relative;top:0px;margin-top:5px;">';
...
if(isset($from_notes_php)){
$echo.='<div class="note_header">By <a hc="" href="/'.$uti['username'].'">'.$uti['fullname'].'</a> &#183; <span title="'.rtd_note($m['datetimep']).'">'.td_fn($m['datetimep']).'</span></div>';	
}
else{
$echo.='
<div class="note_header"><span title="'.rtd_note($m['datetimep']).'">'.td_fn($m['datetimep']).'</span></div>';
}
//By <a href="/'.$uti['username'].'">'.$uti['fullname'].'</a> &#183; 
$echo.='
...
<a href="/notes/'.$flm.'/'.$note_titlem.'/'.$sbid.'">View Full Note</a> &#183;
';

if(in_array($uid,$u_friends_uidv)){
$echo.='<tr><td class="story_foot_td" style="padding-left:2px;"><div class="lbl inlineBlock">'.$like.'<span class="friendslink3v" data-dx="'.$x.'" data-leavecomment="t" title="Leave a comment">Comment</span>&nbsp;</div>'; 
}
...
$r2 = mysql_query("SELECT * FROM commentsv WHERE elemid='$thefabtext' AND type='notes'");
...
if($uid==$uidv){
$no_notes_text='You have not written any notes.<a class="pls" style="font-size:13px!important;" onclick="window.location=\'/editnote.php\'" return false;" href="#">Write a Note</a>';		
}
else{
$no_notes_text='This user has not written any notes.';	
}
$echo.='<div class="clearfix sbxNullState"><img style="display: block;margin-right: 8px;float:left;" height="32" width="32" src="/images/e4B6gExrCqw.png"><div style="display: table-cell;vertical-align: top;"><p style="font-size: 13px;padding:0;margin:0;padding-top: 7px;" class="lb">'.$no_notes_text.'</p></div></div>';	
}
...
if(isset($from_notes_php)){
$echo.='<a href="/notes.php?id='.$uti['id'].'&s='.$enm.'" class="note_left_nav_arrow uiButton nav_arrow_disabled"><i class="note_i_left_nav nav_arrow_disabled_i"></i></a><a href="/notes.php?id='.$uti['id'].'&s='.$en.'" class="note_right_nav_arrow uiButton nav_arrow_disabled"><i class="note_i_right_nav nav_arrow_disabled_i"></i></a>';	
}
else{
$echo.='<a href="/'.$uti['username'].'?sk=notes&s='.$enm.'" class="note_left_nav_arrow uiButton nav_arrow_disabled"><i class="note_i_left_nav nav_arrow_disabled_i"></i></a><a href="/'.$uti['username'].'?sk=notes&s='.$en.'" class="note_right_nav_arrow uiButton nav_arrow_disabled"><i class="note_i_right_nav nav_arrow_disabled_i"></i></a>';
}
$echo.='
</div>
</div>
...
$("#llmn").removeClass("wrapper_fotos");
$("#llmn").addClass("wrapper_fotos2");
$("#llmnv").removeClass("linkwrap");
$("#llmnv").addClass("linkwrap2");
</script>
';
}
else{
$params['left_link_w']='notes';
$params['left_menu_added']='
<script type="text/javascript" data-lma="">
...
$params['left_menu_added']='
<script type="text/javascript" data-lma="">
$("#llm9").parent().eq(0).after(\'<div data-lma=""><a href="/'.$uti['username'].'?sk=notes_tagged"><div id="llmnt" class="wrapper_fotos" style="padding-left:6px;padding-top:3px;"><div id="llmntv" class="linkwrap subitem" style="position:relative;left:25px;display:inline-block;bottom:0;">Notes About</div></div></a></div>\');
</script>
';	
}
...
$echo.='
<script type="text/javascript">
if(note_li_mouseover===undefined){
var note_li_mouseover="";

$(document).on("mouseover",".note_li",function(){
$(this).find(".ocontrol").stop().fadeTo(0,1);
});

$(document).on("mouseout",".note_li",function(){
$(this).find(".ocontrol").stop().fadeTo(0,0);
});
