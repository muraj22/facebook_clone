...
function _mkService($uris = null, $type_uris = null,
                    $delegate = null, $dent = '        ')
{
    $sufs = array($dent, "<Service>\n");
    $dent2 = $dent . '    ';
    if ($type_uris) {
        foreach ($type_uris as $type_uri) {
            $sufs = array_merge($sufs,
                                  array($dent2 . '<Type>',
                                        $type_uri, "</Type>\n"));
        }
    }
    ...
    if ($uris) {
        foreach ($uris as $uri) {
            ...
            $sufs = array_merge($sufs, array($dent2, '<URI'));
            if ($prio !== null) {
                    $sufs = array_merge($sufs, array(' priority="', strval($prio), '"'));
            }
            $sufs = array_merge($sufs, array('>', $uri, "</URI>\n"));
        }
    }
    ...
    $sufs = array_merge($sufs, array($dent, "</Service>\n"));
