...
foreach($active_keys as $key)
{
	...
	$albumn=$m['albumn'];	
	if($albumn!="Wall Photos" && $albumn!="Photos" && $albumn!="Wall Pins" && $albumn!="Pins"){
	exit();	
	}
	}
...
//$new_filter="normal"; //added because we dropped photo filters cause of ram hog
//$tilt_shift=0;
if($new_filter=="normal" && $tilt_shift!=1 || $new_filter=="normal" && $brightness>0 || $new_filter=="normal" && $contrast>0){
