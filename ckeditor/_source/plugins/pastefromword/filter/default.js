	...
	var plugin = ( CKEDITOR.plugins.pastefromword =
	{
						...
						attrs.style = plugin.filters.stylesFilter(
								[
									...
									[ ( /^margin(:?-left)?$/ ), null, function( margin )
									{
										...
										margin = CKEDITOR.tools.converttopxv( values[ 3 ] || values[ 1 ] || values [ 0 ] );
							...
							plugin.filters.stylesFilter(
								[
									[ 'tab-stops', null, function( val )
									{
										...
										margin && ( previousListItemMargin = CKEDITOR.tools.converttopxv( margin[ 0 ] ) );
									} ],
		...
		getRules : function( editor )
		{
			var dtd = CKEDITOR.dtd,
				...
				converttopxv = function( value )
					{
						value = CKEDITOR.tools.converttopxv( value );
					...
					$ : function( element )
					{
							...
							&& attrs.style )
						{
							attrs.style = stylesFilter(
										[ [ ( /^(:?width|height)$/ ), null, converttopxv ] ] )( attrs.style ) || '';
						}
